...
const manualFallbacks = {
  'termalnye-istochniki-yang-bay-iz-nyachanga': {
    schedule: ['7:00 — сбор участников из отелей', '16:00 — возвращение в отель'],
    locations: [
      { name: 'Эко-парк Янг Бэй', description: 'Знакомство с природным парком, термальными зонами и программой комплекса.' },
    ],
    groupPrices: ['Взрослые — 34$', 'Дети — 24$'],
  },
  'dzip-tur-v-nyachange': {
    schedule: ['8:00 — сбор участников из отелей', '16:00 — возвращение в отель'],
    locations: [
      { name: 'Цитадели и исторические объекты', description: 'Знакомство с сохранившимися историческими объектами маршрута.' },
    ],
    groupPrices: ['Взрослые — 90$', 'Дети — 65$'],
  },
  'ekskursiya-baho-zoklet-iz-nyachanga': {
    schedule: ['7:00 — сбор участников из отелей', '16:00 — возвращение в отель'],
    locations: [
      { name: 'Водопад Бахо', description: 'Прогулка по джунглям и посещение природных каскадов.' },
      { name: 'Пляж Зоклет', description: 'Отдых и купание на одном из известных пляжей к северу от Нячанга.' },
    ],
    groupPrices: ['Взрослые — 36$', 'Дети — 26$'],
  },
};
...
async function fetchHtml(sourceUrl) {
  const urls = [sourceUrl, sourceUrl.replace('https://maxtourvietnam.com/', 'https://www.maxtourvietnam.com/')];
  ...
  console.warn(`[facts] source unavailable ${sourceUrl}: ${lastStatus}`);
  return null;
}
...
function collectSection(lines, startPattern, endPatterns) {
  ...
  return lines.slice(start + 1, end).filter((line) => line.length < 180 && !/^забронировать/i.test(line)).slice(0, 12);
}

function fallbackLocations(programLines) {
  const excluded = /цена|сбор участников|возвращение|забронировать|локации|день \d|программа|включено|что взять|важно|взросл|дети|бесплат|\$/i;
  const clean = programLines.filter((line) => !excluded.test(line) && line.length >= 2 && line.length < 220);
  const locations = [];
  for (let index = 0; index < clean.length && locations.length < 16;) {
    const name = clean[index];
    if (name.length > 95) { index += 1; continue; }
    const next = clean[index + 1];
    const description = next && next.length <= 220 ? next : '';
    locations.push(description ? { name, description } : { name });
...
function parseFacts(html, fallback) {
  ...
  const schedule = lines.filter(isSchedule).filter((line, index, all) => all.indexOf(line) === index).slice(0, 6);
  ...
  if (groupMarker >= 0) {
    ...
    groupPrices = priceArea.slice(groupMarker + 1, end).filter((line) => isPriceLine(line) || /цена групповой экскурсии/i.test(line)).slice(0, 14);
  }
  ...
  const lastScheduleIndex = schedule.length ? Math.max(...schedule.map((entry) => lines.indexOf(entry))) : priceAreaEnd;
  const programEndCandidates = [/в стоимость включено/i, /^включено:?$/i, /что входит/i, /что взять/i, /^забронировать тур/i, /часто задаваемые вопросы/i]
  ...
  let locations = locationNames.map((name, index) => {
    ...
    const description = start >= 0 ? programLines.slice(start + 1, end).filter((line) => !excludedStrong.test(line) && line.length < 240).slice(0, 2).join(' ') : '';
    return description ? { name, description } : { name };
  });
  if (!locations.length) locations = fallbackLocations(programLines);
  ...
  return { schedule, locations, groupPrices, privatePrices, included, bring, notice };
}

const generated = {};
for (const [index, tour] of tours.entries()) {
  if (existing.has(tour.slug)) continue;
  const html = await fetchHtml(tour.sourceUrl);
  generated[tour.slug] = html ? parseFacts(html, tour) : (manualFallbacks[tour.slug] || {
    groupPrices: [`Взрослые — ${tour.adultPrice}$`, ...(tour.childPrice !== undefined ? [`Дети — ${tour.childPrice}$`] : [])],
  });
  console.log(`[facts ${index + 1}/${tours.length}] ${tour.slug}: ${generated[tour.slug].locations?.length || 0} locations`);
  await sleep(350);
}
...
for (const [slug, detail] of Object.entries(generated)) {
  ...
  const objectPattern = new RegExp(`(slug:\\s*'${escaped}'[\\s\\S]*?)(sourceUrl:)`);
